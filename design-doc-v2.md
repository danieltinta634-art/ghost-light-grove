...
- Vendor map: **Lionel's general store** (seeds/staples — see below) · Briar = building/construction · Myra = livestock & ranch supplies · Hazel = food/potions · Victor = tech/equipment upgrades · Alastair's museum = collection content, not commerce. **Museum mechanic (canon, 2026-09-21):** Alastair is cursed with an unnaturally long life that cursed artifacts sustain — stated openly as the pitch for donating ("you help him, he helps you"), not a hidden mystery. Full catalog and reward structure in `museum-collection.md`.
...
1. **Farm** — crops keyed to tides and moon phases; signature crops need both (e.g. a flower that only sprouts if planted on a New Moon during a specific tide). **Minori's shrine (canon, 2026-09-19):** offer a crop/farm good daily at her shrine for a small, general luck buff (crop quality/yield, forage rarity, fish rarity, all a little) lasting one day, refreshable, stronger during Harvest-tide. No shop attached — this is her entire mechanical role, unlike every other candidate.
...
3. **Explore/Fight** — TWO distinct combat zones (the west Woods near town stay SAFE):
   ...
   - **Full Moon danger, Deep Woods only (designer canon):** Briar's affliction fully transforms him on the Full Moon, and he deliberately isolates in the Deep Woods so no one in town gets hurt. If the player enters the Deep Woods during a Full Moon, feral-Briar attacks — but the encounter is **unwinnable by design**, a flee/evade beat rather than combat. Unlike the wild risen (no moral weight, already-dead husks), Briar is a person and a marriage candidate, so the game never lets the player damage-trade with him. Full lore in `characters/briar.md`.
...
| **Minori** | Kitsune (fox spirit, female) | Shrine keeper — harvest luck; assists Tobias with festivals | **Marriage candidate** — the playful trickster. Mischievous, teasing, flirtatious; the light, charming foil to the pool's guarded types. Name means "harvest"/"bearing of fruit"; leans into authentic Inari-shrine iconography (kitsune are traditionally Inari's messengers) rather than a moon motif — the cast's one deliberately non-western mythological figure. Lives at her shrine on the manor-to-town path; keeps no shop. One of the outcasts Dorian's amplified song called to the Grove. |
| **Rowan** | Dryad (male nature-spirit / Green Man, Herne-the-Hunter styling) | Woods caretaker (no shop/mechanic) | **Marriage candidate** — the elegant/nurturing one, with a mature, older, reserved-but-caring energy layered on top. Calm, rooted, graceful; antlers, foliage crown, bark-textured skin, soft glowing eyes. Lives inside a large ancient tree in the Woods. Slow-blooming, earthy romance. (Named for the rowan tree.) |
...
- **Center-north:** the manor on its **circular cliff**, back against the skyline, moon arcing overhead. Farm fields on the plateau; stairs descend south through **Cliff's graveyard**, past **Minori's shrine** (sited on this path so her daily luck-offering ritual folds into the manor-to-town commute).
...
- **NPCs: equipped tools/weapons render baked into the character sheet** — each tool gets its own full set of directional-action frames on the sprite sheet, rather than a separate overlay layer or a generic shared swing animation. Works because each NPC has exactly one fixed appearance.
- **The player character is the exception: layered/modular sprites, not baked-per-combo.** Fully customizable (Stardew-style character creator — two body types tied to gender selection, multiple skin tones, several hairstyles/colors, a handful of starting outfits, full Stardew-scale range). Baking every combination the NPC way would multiply out of control, so the player is built from a base body layer + swappable hair layer + swappable outfit layer + a separate tool/weapon overlay layer, composited at runtime — each authored once per option rather than once per combination. The player's appearance is otherwise completely ordinary (no visual hint of the reaper bloodline; the pendant does all the reveal work). Full breakdown in `characters/art/player.md`.
...
**World/Systems:** tide names & flavors (proposed, unapproved) · scythe upgrade source · cave floor structure · likes/dislikes pass · arrival-story one-liners per resident (Rowan's specifically still open) · whether ghost light doubles as reaper-rank XP (see Economy).
