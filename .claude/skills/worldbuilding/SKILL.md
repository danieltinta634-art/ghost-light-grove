...
> **Adapted for this repo:** Ghost Light Grove doesn't use the story-init scaffold — treat `design-doc-v2.md` as the story bible (it already covers World Rules, the map, and the vendor/economy layout). There's no `worldbuilding/` directory, `_index.md` registry, or `story` CLI here yet; when this skill is used, create new location/system/faction/artifact files directly and cross-reference them from `design-doc-v2.md` and the relevant `characters/*.md` files by hand instead of running the CLI steps below.
