...
> **Adapted for this repo:** Ghost Light Grove doesn't use the story-init scaffold. Treat `design-doc-v2.md` and `story-spoiler-doc.md` as the existing plot/lore record — story-spoiler-doc.md already holds the hard spoilers (the reaper mantle, the goddess beneath the well, the replacement-reaper antagonist), so keep new spoiler content there and keep `design-doc-v2.md` spoiler-light, matching this repo's existing convention. There's no `plot/` directory, `_index.md`, or `story` CLI here; create arc/timeline/promise files directly and cross-reference `characters/*.md` by hand instead of running the CLI steps below.
