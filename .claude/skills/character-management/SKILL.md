...
> **Adapted for this repo:** Ghost Light Grove doesn't use the story-init scaffold. Treat `design-doc-v2.md` as the story bible and the existing `characters/*.md` files as the character files — use `characters/_TEMPLATE.md`'s frontmatter schema instead of `references/character-template.md` below when the two disagree. There's no `characters/_index.md` registry or `story` CLI here, so skip those steps; just keep `links:` fields in character frontmatter bidirectional by hand.
